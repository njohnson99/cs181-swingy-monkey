...
class Learner(object):
    ...
    def __init__(self):
        ...
        self.game_num = 1
        self.Q = np.zeros((4, 4, 4, 4, 2, 2, 2)) # because there are 2 actions
        self.epsilon = 0.8
        self.gravity = None
    ...
    def discretize_state(self, state):
        ...
        tree_top_gap = (state['tree']['top'] - state['monkey']['top']) / H
        tree_bot_gap = (state['tree']['bot'] - state['monkey']['bot']) / H
        ...
        return int(tree_dist), int(tree_top_gap), int(tree_bot_gap), int(screen_gap), int(velocity)
    ...
    def action_callback(self, state):
        ...
        if self.last_state != None and state != None:
            ...
            tree_dist_1, tree_top_gap_1, tree_bot_gap_1, screen_gap_1, velocity_1 = self.discretize_state(self.last_state)
            tree_dist_2, tree_top_gap_2, tree_bot_gap_2, screen_gap_2, velocity_2 = self.discretize_state(state)
            ...
            if np.random.binomial(1, self.epsilon) == 1:
                ...
                new_action = np.argmax(self.Q[tree_dist_2][tree_top_gap_2][tree_bot_gap_2][screen_gap_2][velocity_2][self.gravity])
                #if new_action == 1:
                    #print self.Q[tree_dist_2][tree_top_gap_2][tree_bot_gap_2][screen_gap_2][velocity_2][self.gravity]

            self.Q[tree_dist_1][tree_top_gap_1][tree_bot_gap_1][screen_gap_1][velocity_1][self.gravity][self.last_action] = \
            (1 - learning) * self.Q[tree_dist_1][tree_top_gap_1][tree_bot_gap_1][screen_gap_1][velocity_1][self.gravity][self.last_action] + \
            learning * (self.last_reward + discount * max(self.Q[tree_dist_2][tree_top_gap_2][tree_bot_gap_2][screen_gap_2][velocity_2][self.gravity]))
...
def run_games(learner, hist, iters = 100, t_len = 100):
    ...
    plt.scatter(range(0, len(hist)), hist)
    plt.xlabel('Epoch')
    plt.ylabel('Score')
    plt.title('Score over time')
    plt.savefig('scoreplot_d25_l3.png')

    np.savetxt("decrease_3.csv", hist, delimiter=",")
